...
public:
	UI_PortPathDialog(const SString &port_name, const SString &openFolder) :
		UI_Escapable_Window(560, 250, "Port Settings"),
		want_close(false), openFolder(openFolder)
	{
...
static SString GrabWadNames(const Instance &inst, const port_path_info_t *info)
{
	...
	AppendWadName(wad_names, inst.master.game_wad->PathName(), "-iwad");

	// add any resource wads
	for (const std::shared_ptr<Wad_file> &wad : inst.master.dir)
	{
		if (wad == inst.master.game_wad || wad == inst.master.edit_wad)
			continue;
	...
	if (inst.master.edit_wad)
		AppendWadName(wad_names, inst.master.edit_wad->PathName(), !has_file ? "-file" : NULL);
