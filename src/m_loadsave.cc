...
bool Instance::Project_AskFile(SString &filename) const
{
	...
	chooser.directory(master.fileOpFolder().c_str());
...
void Instance::CMD_NewProject()
{
	...
	if (idx >= 0)
	{
		...
		map_name = master.game_wad->GetLump(idx)->Name();
	}
...
void Instance::CMD_FreshMap()
{
	...
	if (master.edit_wad->LevelFind(map_name) >= 0)
	{
...
{
	Lump_c *lump = Load_LookupAndSeek(loading_level, load_wad, "VERTEXES");
	if (! lump)
...
{
	Lump_c *lump = Load_LookupAndSeek(loading_level, load_wad, "SECTORS");
	if (! lump)
...
void Instance::LoadBehavior(int loading_level, const Wad_file *load_wad)
{
	...
	Lump_c *lump = Load_LookupAndSeek(loading_level, load_wad, "BEHAVIOR");
	if (! lump)
...
void Instance::LoadScripts(int loading_level, const Wad_file *load_wad)
{
	...
	Lump_c *lump = Load_LookupAndSeek(loading_level, load_wad, "SCRIPTS");
	if (! lump)
		return;
...
{
	Lump_c *lump = Load_LookupAndSeek(loading_level, load_wad, "THINGS");
	if (! lump)
...
void OpenFileMap(const SString &filename, const SString &map_namem)
{
	...
	{
		if (! M_ParseEurekaLump(gInstance.loaded, wad.get()))
		{
			return;
		}
	}
	...
	SYS_ASSERT(gInstance.master.edit_wad == wad);


	// always grab map_name from the actual level
	{
		int idx = gInstance.master.edit_wad->LevelHeader(lev_num);
		map_name  = gInstance.master.edit_wad->GetLump(idx)->Name();
	}
...
void Instance::CMD_OpenMap()
{
	...
	{
		if (! M_ParseEurekaLump(loaded, wad.get()))
			return;
	}
	...
	if (did_load)
	{
		SYS_ASSERT(wad != master.edit_wad);
		SYS_ASSERT(wad != master.game_wad);
...
void Instance::SaveLevel(const SString &level)
{
	...
	if (lev_num >= 0)
	{
		level_lump = master.edit_wad->LevelHeader(lev_num);

		master.edit_wad->RemoveLevel(lev_num);
	}

	master.edit_wad->InsertPoint(level_lump);
	...
	if (main_win)
	{
		main_win->SetTitle(master.edit_wad->PathName(), loaded.levelName, false);
...
bool Instance::M_ExportMap()
{
	...
	if (master.haveFilename(filename))
	{
		DLG_Notify("Unable to export the map:\n\nFile already in use");
		return false;
	}
...
void Instance::CMD_RenameMap()
{
	...
	// perform the rename
	int lev_num = master.edit_wad->LevelFind(loaded.levelName);
...
void Instance::CMD_DeleteMap()
{
	...
	if (master.edit_wad->LevelCount() < 2)
	{
	...
	{
		if (lev_num >= master.edit_wad->LevelCount())
			lev_num = master.edit_wad->LevelCount() - 1;

		int lump_idx = master.edit_wad->LevelHeader(lev_num);
		Lump_c * lump  = master.edit_wad->GetLump(lump_idx);
		const SString &map_name = lump->Name();
