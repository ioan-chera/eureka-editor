...
void WadData::updateGamma() noexcept
{
	for (int c = 0 ; c < 256 ; c++)
	{
...
void WadData::loadPalette(const MasterDirectory &master) noexcept(false)
{
...
void WadData::loadColormap(const MasterDirectory &master) noexcept(false)
{
	Lump_c *lump = master.findGlobalLump("COLORMAP");
...
byte WadData::findPaletteColor(int r, int g, int b) const noexcept
{
	int best = 0;
	int best_dist = (1 << 30);
...
static void W_CreateBrightMap(WadData &wad)
{
	for (int c = 0 ; c < 256 ; c++)
	{
		...
		wad.bright_map[c] = wad.findPaletteColor(r, g, b);
	}
}
