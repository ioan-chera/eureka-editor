...
static void UDMF_ParseGlobalVar(LoadingData &loading, Udmf_Parser& parser,
								Udmf_Token& name)
{
...
static void UDMF_ParseObject(Document &doc, Udmf_Parser& parser, Udmf_Token& name)
{
	...
	else if (name.Match("vertex"))
	{
		...
		new_V = new Vertex;
		doc.vertices.push_back(std::unique_ptr<Vertex>(new_V));
	}
	else if (name.Match("linedef"))
	{
		kind = Objid(ObjType::linedefs, 1);
		new_LD = new LineDef;
		doc.linedefs.push_back(std::unique_ptr<LineDef>(new_LD));
	}
...
void Instance::ValidateLevel_UDMF(LevelLoadProblems &problems)
{
	...
	for (int n = 0 ; n < level.numLinedefs(); n++)
	{
		...
		ValidateVertexRefs(L.get(), n, problems);
		ValidateSidedefRefs(L.get(), n, problems);
	}
}
...
static void UDMF_WriteVertices(const Document &doc, Lump_c *lump)
{
	for (int i = 0 ; i < doc.numVertices(); i++)
	{
		...
		const Vertex *vert = doc.vertices[i].get();
...
{
	Lump_c *lump = master.edit_wad->AddLump("TEXTMAP");
	...
	master.edit_wad->AddLump("ENDMAP");
}
