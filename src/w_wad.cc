...
int loadLumpData(Lump_c &lump, std::vector<byte> &buffer)
{
	...
	if (lump.Length() > 0)
	{
		...
		{
			ParseException::raise("%s: read error loading lump '%s'.",
								  __func__, lump.Name().c_str());
		}
	}
...
void MasterDirectory::add(const std::shared_ptr<Wad_file> &wad)
{
...
void MasterDirectory::remove(const std::shared_ptr<Wad_file> &wad)
{
