								...
								int j, int k, bool do_right) const
{
	...
	if (((ob_j.parts & PART_LF_ALL) ? 0 : 1) == (do_right ? 1 : 0))
		vj = doc.getLinedef(ob_j.num).end;
	else
		vj = doc.getLinedef(ob_j.num).start;

	int vk = 0;

	if (((ob_k.parts & PART_LF_ALL) ? 0 : 1) == (do_right ? 1 : 0))
		vk = doc.getLinedef(ob_k.num).start;
	else
		vk = doc.getLinedef(ob_k.num).end;
...
{
	int old_start = doc.getLinedef(ld).start;
	int old_end   = doc.getLinedef(ld).end;
...
{
	int old_right = doc.getLinedef(ld).right;
	int old_left  = doc.getLinedef(ld).left;
...
int LinedefModule::splitLinedefAtVertex(EditOperation &op, int ld, int new_v) const
{
	...
	auto &L2 = doc.getMutableLinedef(new_l);
...
void Instance::commandLinedefMergeTwo()
{
	...
	const auto & L1 = level.getLinedef(ld1);
	const auto & L2 = level.getLinedef(ld2);
...
bool LinedefModule::linedefStartWillBeMoved(int ld, selection_c &list) const
{
	for (sel_iter_c it(list) ; !it.done() ; it.next())
	{
		...
		if (*it != ld && L.end == doc.getLinedef(ld).start)
			return true;
	}
...
bool LinedefModule::linedefEndWillBeMoved(int ld, selection_c &list) const
{
	for (sel_iter_c it(list) ; !it.done() ; it.next())
	{
		...
		if (*it != ld && L.start == doc.getLinedef(ld).end)
			return true;
	}
...
void LinedefModule::linedefSetLength(EditOperation &op, int ld, int new_len, double angle) const
{
	...
	if (new_len < 0)
	{
		op.changeVertex(L.start, &Vertex::raw_x, doc.getEnd(L).raw_x - FFixedPoint(idx));
		op.changeVertex(L.start, &Vertex::raw_y, doc.getEnd(L).raw_y - FFixedPoint(idy));
	}
	else
	{
		op.changeVertex(L.end, &Vertex::raw_x, doc.getStart(L).raw_x + FFixedPoint(idx));
		op.changeVertex(L.end, &Vertex::raw_y, doc.getStart(L).raw_y + FFixedPoint(idy));
	}
}
