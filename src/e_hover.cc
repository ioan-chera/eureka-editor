...
//  is chosen.
//
Objid Instance::getNearbyObject(ObjType type, const v2double_t &pos) const
{
	switch(type)
	{
	case ObjType::things:
	...
	case ObjType::sectors:
		return hover::getNearestSector(level, pos);
...
Objid Instance::findSplitLine(v2double_t &out_pos, const v2double_t &ptr, int ignore_vert, bool exactPoint) const
{
	...
	else if(!exactPoint && grid.snaps())
	{
		...
		if(level.touchesCoord(*L, out_pos.x, out_pos.y))
			return Objid();
	...
	v2double_t p2, int possible_v2) const
{
	...
	for(int v = 0; v < doc.numVertices(); v++)
	{
		...
		if(VC->Matches(p1.x, p1.y) || VC->Matches(p2.x, p2.y))
			continue;
