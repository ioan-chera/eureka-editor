...
# endif
	...
	{
		auto &L = doc.getMutableLinedef(i);
...
static void MarkPolyobjPoint(double x, double y, Instance &inst)
{
	...
	{
		const auto &L = inst.level.getLinedef(i);
...
#     endif
	...
	const auto &best_ld = inst.level.getLinedef(best_match);
...
# endif
	...
	{
		const auto &T = inst.level.getThing(i);
...
static FFixedPoint LineStartCompare(const Document &doc, const void *p1, const void *p2)
{
	...
	const auto &A = doc.getLinedef(line1);
	const auto &B = doc.getLinedef(line2);
	...
	for (i=0 ; i < doc.numLinedefs() - 1 ; i++)
	{
		...
		for (j = i+1 ; j < doc.numLinedefs(); j++)
		{
			...
			if (LineEndCompare(doc, array + i, array + j).raw() == 0)
			{
				...
				auto &L = doc.getMutableLinedef(array[j]);
...
vertex_t *NewVertexFromSplitSeg(seg_t *seg, double x, double y, const Document &doc)
{
	...
	if (seg->linedef < 0 || doc.getLinedef(seg->linedef).TwoSided())
	{
