...
static std::string FindDefinitionFile(const char *folder, const char *name)
{
	...
	{
		const char *base_dir = (pass == 0) ? config::home_dir.c_str() : config::install_dir.c_str();
...
//	DebugPrintf("M_CollectKnownDefs for: %d\n", folder);
	...
	snprintf(path, sizeof(path), "%s/%s", config::install_dir.c_str(), folder);
	ScanDirectory(path, scanner_add_file, & temp_list);

	snprintf(path, sizeof(path), "%s/%s", config::home_dir.c_str(), folder);
	ScanDirectory(path, scanner_add_file, & temp_list);
