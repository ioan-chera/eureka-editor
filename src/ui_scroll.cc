...
{
	if (last_bounds[0] == mInstance.level.Map_bound_x1 &&
		last_bounds[1] == mInstance.level.Map_bound_x2)
	{
		return;
	}

	last_bounds[0] = static_cast<int>(mInstance.level.Map_bound_x1);
	last_bounds[1] = static_cast<int>(mInstance.level.Map_bound_x2);

	int expand = static_cast<int>(512 + (mInstance.level.Map_bound_x2 - mInstance.level.Map_bound_x1) / 8);

	bound_x1 = static_cast<int>(mInstance.level.Map_bound_x1 - expand);
	bound_x2 = static_cast<int>(mInstance.level.Map_bound_x2 + expand);
...
{
	if (last_bounds[2] == mInstance.level.Map_bound_y1 &&
		last_bounds[3] == mInstance.level.Map_bound_y2)
	{
		return;
	}

	last_bounds[2] = static_cast<int>(mInstance.level.Map_bound_y1);
	last_bounds[3] = static_cast<int>(mInstance.level.Map_bound_y2);

	int expand = static_cast<int>(512 + (mInstance.level.Map_bound_y2 - mInstance.level.Map_bound_y1) / 8);

	bound_y1 = static_cast<int>(mInstance.level.Map_bound_y1 - expand);
	bound_y2 = static_cast<int>(mInstance.level.Map_bound_y2 + expand);
...
void UI_CanvasScroll::Adjust_X()
{
	...
	int map_x = static_cast<int>(mInstance.grid.orig_x - map_w / 2);
...
void UI_CanvasScroll::Adjust_Y()
{
	...
	int map_y = static_cast<int>(mInstance.grid.orig_y - map_h / 2);
...
void UI_CanvasScroll::Scroll_X()
{
	...
	mInstance.grid.MoveTo(new_x, mInstance.grid.orig_y);
}
...
void UI_CanvasScroll::Scroll_Y()
{
	...
	mInstance.grid.MoveTo(mInstance.grid.orig_x, new_y);
}
