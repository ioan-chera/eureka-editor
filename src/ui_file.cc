	...
	UI_Escapable_Window(420, 475, "Open Map"), inst(inst)
{
...

	{
		...
		look_where->value(inst.wad.edit_wad ? LOOK_PWad : LOOK_IWad);
	}
...
std::shared_ptr<Wad_file> UI_OpenMap::Run(SString* map_v, bool * did_load)
{
	...
	*did_load = false;
	...
	if (inst.wad.edit_wad)
		SetPWAD(inst.wad.edit_wad->PathName());
...
void UI_OpenMap::Populate()
{
	...
	{
		using_wad = inst.wad.game_wad;
		PopulateButtons();
	}
	else if (look_where->value() >= LOOK_Resource)
	{
		int first = 1;
		int last  = (int)inst.wad.master_dir.size() - 1;
		...
		{
			if (inst.wad.master_dir[r]->LevelCount() >= 0)
			{
				using_wad = inst.wad.master_dir[r];
				PopulateButtons();
				break;
			}
		}
	...
	else if (inst.wad.edit_wad)
	{
		using_wad = inst.wad.edit_wad;
		PopulateButtons();
	}
